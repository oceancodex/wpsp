...
return new class extends Migration
{
	...
	{
		$teams = Funcs::config('permission.teams');
		$tableNames = Funcs::config('permission.table_names');
		$columnNames = Funcs::config('permission.column_names');
		...
		if (! Schema::hasColumn($tableNames['model_has_permissions'], $columnNames['team_foreign_key'])) {
			Schema::table($tableNames['model_has_permissions'], function (Blueprint $table) use ($tableNames, $columnNames, $pivotPermission) {
				...
				if (Funcs::app('db')::getDriverName() !== 'sqlite') {
		...
		Funcs::app('cache')
			->store(Funcs::config('permission.cache.store') != 'default' ? Funcs::config('permission.cache.store') : null)
			->forget(Funcs::config('permission.cache.key'));
	}
