...
//	protected $connection = 'mongodb';
    ...
    public function up(Schema $schema): void {
        $tblJobs        = Funcs::getDBCustomMigrationTableName('jobs');
        $tblFailedJobs  = Funcs::getDBCustomMigrationTableName('failed_jobs');
        $tblJobBatches  = Funcs::getDBCustomMigrationTableName('job_batches');

        // jobs
        if (!$schema->hasTable($tblJobs)) {
            $jobs = $schema->createTable($tblJobs);
        ...
        // failed_jobs
        if (!$schema->hasTable($tblFailedJobs)) {
            $failed = $schema->createTable($tblFailedJobs);
        ...
        // job_batches
        if (!$schema->hasTable($tblJobBatches)) {
            $batches = $schema->createTable($tblJobBatches);
    ...
    public function down(Schema $schema): void {
        $tblJobs        = Funcs::getDBTableName('jobs');
        $tblFailedJobs  = Funcs::getDBTableName('failed_jobs');
        $tblJobBatches  = Funcs::getDBTableName('job_batches');

        if ($schema->hasTable($tblJobBatches)) $schema->dropTable($tblJobBatches);
        if ($schema->hasTable($tblFailedJobs)) $schema->dropTable($tblFailedJobs);
        if ($schema->hasTable($tblJobs)) $schema->dropTable($tblJobs);
    }
}
