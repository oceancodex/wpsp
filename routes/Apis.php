...
class Apis {
	...
	public function apis() {
		Route::namespace('wpsp')
			->version('v1')
			->group(function() {
				Route::name('api-token.')->prefix('api-token')->group(function() {
//					Route::post('get', [ApisController::class, 'getApiToken'])->name('get');
					Route::middleware([ApiTokenAuthentication::class, AuthenticationMiddleware::class])->post('test', [ApisController::class, 'testApiToken'])->name('test');
				});
...
////						Route::post('(?P<id>\d+)/update', [ApisController::class, 'usersUpdate'])->name('update');
