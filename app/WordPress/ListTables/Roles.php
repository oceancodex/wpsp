...
//	public $screenOptionsKey = null;
	...
	public function process_bulk_action() {
		...
		if (!empty($_REQUEST['_wpnonce']) && $nonce = $_REQUEST['_wpnonce']) {
			...
			if ('delete' === $this->current_action()) {
				...
				if (!empty($items)) {
					RoleModel::query()->whereIn('id', $items)->delete();
				}
	...
	public function column_name($item) {
		$actions = [
			...
			'show'   => '<a href="' . Funcs::route('AdminPages', 'wpsp.roles.show', ['role' => $item['id']], true) . '">Show</a>',
			'edit'   => '<a href="' . Funcs::route('AdminPages', 'wpsp.roles.edit', ['id' => $item['id']], true) . '">Edit</a>',
			'delete' => '<a href="' . Funcs::route('AdminPages', 'wpsp.roles.delete', ['id' => $item['id']], true) . '">Delete</a>',
		];
